...
begin
  ...
  { intro h,
    ...
    { have : ∀ n : ℕ, ∃ a ∈ A, x - 1/(n+1) < a,
      ...
      split,
      { apply squeeze,
        { exact limit_const_sub_inv_succ x, },
        { exact limit_const_add_inv_succ x, },
        { exact λ n, le_of_lt (hu n).right, },
        { intro n,
          have : u n ≤ x,
          {  apply h.1,
             exact (hu n).1, },
          have : x < x + 1 / (n+1) := lt_add_of_pos_right x nat.one_div_pos_of_nat,
          linarith, } },
      { exact λ n, (hu n).left, },
  } },
  { rintro ⟨maj, u, limu, u_in⟩, 
    ...
    { intros y hy,
      apply le_of_le_add_all,
      intros ε ε_pos,
      cases limu ε ε_pos with N hN,
      have : u N - x ≥ -ε := (abs_le.mp (hN N (le_refl N))).left,
      have : u N ≤ y := hy (u N) (u_in N),
      linarith, },
  },
end
